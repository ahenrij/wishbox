            ...
            <div class="col-md-9">
                ...
                <div class="uk-card uk-card-default uk-card-body">
                    ...
                    <div uk-grid>
                        ...
                        <div class="uk-width-expand@s">
                            ...
                            @if($wish->status == WISH_ON_THE_WAY)
                                <a href="{{ route($type.'.received', [$wish->id]) }}"
                                   class="btn btn-outline-success">{{ __('J\'ai reçu !') }}</a>
                            @endif

                            @if (Auth::user()->id != $wish->wishBox->user_id)
                                @if($wish->wishBox->type == TYPE_WISH)
                                    @if($wish->user_id == null)
                                        <a href="{{ route('wish.offer.wish', $wish->id) }}"
                                           class="btn btn-outline-success pr-5 pl-5"
                    ...
                    @if (Auth::user()->id != $wish->wishBox->user_id && $wish->wishBox->type == TYPE_GIFT && $wish->status == WISH_JUST_CREATED)
                        <form name="obtainGift" action="{{ route('gift.get', $wish->id) }}">
                            ...
                            <button type="submit" class="btn btn-primary hoverShadow mt-2 pl-5 pr-5">Obtenir</button>
                        </form>
                    @endif
                </div>
                ...
                @if($wish->wishBox->type == TYPE_GIFT && $wish->wishBox->user_id == \Illuminate\Support\Facades\Auth::user()->id)
                    <div class="uk-card uk-card-default uk-card-body mt-4" id="comments">
                            ...
                            <div>
                                @foreach($comments as $comment)
                                    ...
                                    <div class="media comment-box mb-2" id="comment-{{ $comment->user_id }}">
                                        <img class="align-self-center rounded-circle mr-3 ml-1 hoverShadow"
                                             src="{{ $comment->profile }}" alt="Profil"
                                             width="150" height="150">
                                        <div class="media-body">
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <h5 class="mt-0 text-left"><a class="profile-link"
                                                                                  href="{{ route('user.show', $comment->user_id) }}">{{ $comment->username }}</a>
                                                    </h5>
                                                </div>
                                            ...
                                            {{--Si le cadeau est déjà offert ne plus afficher le bouton offrir--}}
                                            @if($wish->status == WISH_JUST_CREATED)
                                                ...
                                                >
                                                    <button class="btn btn-primary hoverShadow mb-2"><i
                                                                class="fa fa-heart"></i> Offrir
                                                    </button>
                                                </a>
                                            @endif
                        ...
                        @else
                            <p class="text-center">Aucune manifestation d'intérêt pour le moment. Vous pouvez partager
                                le lien vers votre
                                cadeau sur vos réseaux sociaux pour qu'il ait plus de visibilité.
                                <br>
                                <button onclick='copyToClipboard("{{ route("wish.show", $wish->id) }}")'
                                        class="btn btn-primary"><i class="fa fa-copy"></i> Copier le lien dans le presse
                                    papier
                                </button>
                            </p>
                        @endif
